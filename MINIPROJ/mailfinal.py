...
class Signup(QMainWindow):
    ...
    def signup(self):
        ...
        if not self.validate_email(email):
            QMessageBox.critical(self, "Error", "Please enter a valid SFIT email address (e.g., john@student.sfit.ac.in)")
            return
    ...
    def validate_email(self, email):
        ...
        return email.endswith("@student.sfit.ac.in")
...
class Studentjoin(QMainWindow):
    ...
    def attemptquiz(self):
        ...
        if code:  # Checking if the input is empty
            ...
            if self.check_code_correctness(code):
                attempt_quiz = Attemptquiz(code)
                widget.addWidget(attempt_quiz)
                widget.setCurrentIndex(widget.currentIndex() + 6)
            else:
                QMessageBox.critical(self, "Error", "Incorrect code. Please enter a valid code.")
        else:
    ...
    def check_code_correctness(self, code):
        try:
            ...
            # Execute a query to check if the code exists in the database
            query = "SELECT COUNT(*) FROM quiz_que WHERE code = %s"
            cursor.execute(query, (code,))
            ...
            # If count is greater than 0, it means the code exists
            if count > 0:
                return True
            else:
                return False
...
class Createque(QMainWindow):
    ...
    # Check if there are questions to save after adding pending question
        ...
        cursor = conn.cursor()
        query = "INSERT INTO quiz_que (question, option1, option2, option3, option4, correct_option, code, qno) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
...
class ExcelViewer(QMainWindow):
    ...
    def save_to_database(self):
        ...
        for row in range(self.tableWidget.rowCount()):
            ...
            query = "INSERT INTO quiz_que (question, option1, option2, option3, option4, correct_option,code,qno) VALUES (%s,%s, %s, %s, %s, %s, %s,%s)"
            data = (question_text, option1, option2, option3, option4, correct_option,code,row+1)
            cursor.execute(query, data)
